...
fn migrate_bootstrap_json_to_env(env_path: &std::path::Path) {
    ...
    if let Some(url) = parsed.get("database_url").and_then(|v| v.as_str()) {
        // Write directly to env_path (not via save_database_url, which
        // targets the global path and wouldn't match in tests)
        ...
        // Rename old bootstrap.json so we don't re-migrate
